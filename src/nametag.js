...
function paintTag(ctx, name, cross) {
  ...
  let px = BASE_FONT_PX;
  ctx.font = font(px);
  // Only the cross label can overflow, and only the cross path measures —
  // this keeps the neutral path's call sequence (and its cost) untouched.
  // measureText is guarded because the tag is built against fake 2D contexts
  // in headless tests.
  if (cross && typeof ctx.measureText === 'function') {
